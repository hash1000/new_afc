  ...
  return (
    <>
      ...
      <div className="container-page mx-auto mt-6 flex flex-col gap-10 pb-16 sm:mt-8 sm:gap-14">
        <Link
          href="/food-menu"
          className="text-brand-navy inline-flex w-fit items-center gap-1.5 text-xs font-semibold hover:opacity-70 sm:text-sm"
        >
        ...
        {sections.map((section) => (
          <BrandMenuSection key={section.id} section={section} image={brand.foodImage} />
        ))}
      </div>
    </>
  );
